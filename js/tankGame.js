...
function create() {
  this.physics.world.on('worldbounds',function(body){killBullet(body.gameObject)}, this);
  ...
  this.physics.world.setBounds(0, 0, this.map.widthInPixels, this.map.heightInPixels);
	WorldSizeW = this.map.widthInPixels;
	WorldSizeH = this.map.heightInPixels;
	...
	navMesh = this.navMeshPlugin.buildMeshFromTiled("mesh",MeshLayer,12.5);
	//console.log(path); 
    ...
    for(var i = 0; i < maxEnemies; i++){
      ...
      enemyTank = new EnemyTank(this, loc.x, loc.y, 'enemy', 'tank1', player, i);
      enemyTank.enableCollision(destructLayer);
      enemyTank.setBullets(enemyBullets);
      enemyTanks.push(enemyTank);
	  enemyTank.target = new Phaser.Geom.Circle.Random(player.bubble);
	  enemyTank.target.x = Math.floor(Phaser.Math.Clamp(enemyTank.target.x,0,WorldSizeW));
	  enemyTank.target.y = Math.floor(Phaser.Math.Clamp(enemyTank.target.y,0,WorldSizeH));
...
function damageWall(bullet, tile){
  if(bullet != null){killBullet(bullet);}
  var destructLayer = this.map.getLayer("destructable").tilemapLayer;
